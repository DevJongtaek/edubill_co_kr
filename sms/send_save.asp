...
'response.end
	...
	do while not xRs.eof 
		...
		if xRs(3) = "lgt" then
			...
			QSql = QSql & "('" & tmobile & "','7575','1',getdate(),'http://www.p-114.co.kr/wap/index019.asp ## 핸드폰주문##','5','" & idx & i & "')"

		elseif xRs(3) = "ktf" then

			QSql = "INSERT INTO em_tran "
			QSql = QSql & "(tran_phone, tran_callback, tran_status, tran_date, tran_msg, tran_type, tran_id) values " 
			QSql = QSql & "('" & tmobile & "','7575','1',getdate(),'http://www.p-114.co.kr/wap/index016.asp ## 핸드폰주문##','5','" & idx & i & "')"

		else
			QSql = "INSERT INTO em_tran "
			QSql = QSql & "(tran_phone, tran_callback, tran_status, tran_date, tran_msg, tran_type, tran_id) values " 
			QSql = QSql & "('" & tmobile & "','7575','1',getdate(),'http://www.p-114.co.kr/wap/index011.asp ## 핸드폰주문##','5','" & idx & i & "')"
		
		end if

		set db1 = server.CreateObject("ADODB.Connection")   '서버객체 db연결
		db1.Open "provider=sqloledb;server=61.97.113.73;database=MYPG;uid=sa;pwd=copyroom;"	'db를 연다
