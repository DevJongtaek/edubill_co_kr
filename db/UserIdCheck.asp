...
<%
	...
	select case imsiidcount
		case "0"
			imsiidcountmsg = "<font color=#990033><B>"&imsi_userid&"</b></font> 는 사용 <font color=red>가능한</font> 아이디입니다."
		case else
			imsiidcountmsg = "<font color=#990033><B>"&imsi_userid&"</b></font> 는 사용 <font color=red>불가능한</font> 아이디입니다."
	end select
%>

<html>
<head>
<title>::: 회원아이디 확인 :::</title>
...
function checkValue() {
	if (form.userid.value=="") {
		...
		alert("ID는 4자리 이상 10자리 미만으로 기입하셔야 합니다.");
		form.userid.focus();
		return false;
	}
	form.submit() ;
...
<form action="UserIdCheck.asp" method=post name=form onsubmit="return checkValue();">
              				...
              				<tr>
						<td width="199" align="center">아이디 : <input name="userid" type="text" size="16" maxlength="10" style="background-color:#ffffff;border:1 solid #999999; width:130px" value="<%=imsi_userid%>"></td>
