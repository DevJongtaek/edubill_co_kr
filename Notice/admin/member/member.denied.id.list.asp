...
<%
	...
	SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_DENIED_ID_LIST] ")

	intTotalCount = RS(0)
	intPageCount = INT((intTotalCount - 1) / intPageSize) + 1
%>
<script type="text/javascript" src="member/js/member.denied.id.list.js"></script>
		<div id="content">
			<div id="subHead">
				<h3><%=objXmlLang("page_title")%></h3>
				<div class="right_area">
					<span class="button medium icon"><span class="add"></span><button id="btn_deniedid_add" type="button"><%=objXmlLang("btn_deniedid_add")%></button></span>
				</div>
			</div>
			...
			<input type="hidden" id="intPage" value="<%=intPage%>">
				...
				<colgroup>
					<col width="1" /><col width="22" /><col width="60" /><col width="150" /><col /><col width="100" />
				</colgroup>
					<thead>
						<tr>
							<th class="lineL"></th>
							...
							<th class="bar lineR"><%=objXmlLang("list_regdate")%></th>
						</tr>
					</thead>
<%
	SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_DENIED_ID_LIST] 'L', '" & intPage & "', '" & intPageSize & "' ")
...
%>
						<tr>
							...
							<td class="detail"><%=intNumber%></td>
							<td class="lfpd"><%=RS("strUserID")%></td>
							<td class="detail"><%=RS("strDescription")%></td>
...
%>
				...
				<div id="memberdeniedIdAddBody" style="padding-top:30px;"></div>
			</div>
		</div>
