...
<%
	...
	CASE "1"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '1', '" & strSearchType & "', '" & REPLACE(strStartDate, ".", "-") & "', '" & REPLACE(strEndDate, ".", "-") & "' ")
	CASE "2"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '1', '" & strSearchType & "', '" & intYear & "-01-01" & "', '' ")
	CASE "3"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '1', '" & strSearchType & "', '" & YEAR(NOW) & "-" & intMonth & "-01" & "', '' ")
	CASE "4", ""
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '1', '" & strSearchType & "', '', '' ")
	END SELECT

	intTotalCount = RS(0)
%>
<script type="text/javascript" src="../js/Calendar.js"></script>
<script type="text/javascript" src="stat/js/stat.member.join.area.js"></script>
		<div id="content">
			<form id="theForm" method="post" action="?act=statmemberjoinarea">
			<div id="subHead">
				<h3><%=objXmlLang("page_title_join_area")%></h3>
			</div>
			<p class="subHeadDesc"><%=objXmlLang("page_description_join_area")%></p>
...
<%
	SELECT CASE strSearchType
	CASE "1"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '2', '" & strSearchType & "', '" & REPLACE(strStartDate, ".", "-") & "', '" & REPLACE(strEndDate, ".", "-") & "' ")
	CASE "2"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '2', '" & strSearchType & "', '" & intYear & "-01-01" & "', '' ")
	CASE "3"
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '2', '" & strSearchType & "', '" & YEAR(NOW) & "-" & intMonth & "-01" & "', '' ")
	CASE "4", ""
		SET RS = DBCON.EXECUTE("[ARTY30_SP_MEMBER_STAT_AREA] '2', '" & strSearchType & "', '', '' ")
	END SELECT
