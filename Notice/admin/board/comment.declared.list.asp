...
<%
	...
	SET RS = DBCON.EXECUTE("[ARTY30_SP_DECLARED_LIST] 'C', 'C' ")

	intTotalCount = RS(0)
	intPageCount = INT((intTotalCount - 1) / intPageSize) + 1
%>
<script type="text/javascript" src="board/js/comment.declared.list.js"></script>
		<div id="content">
			<form id="theForm" method="post" action="?act=commentdeclared">
...
<%
	SET RS = DBCON.EXECUTE("[ARTY30_SP_DECLARED_LIST] 'L', 'C', '" & intPage & "', '" & intPageSize & "' ")
...
%>
						<tr>
							<td class="detail"></td>
							<td class="chk"><input type="checkbox" id="intSeq" name="intSeq" value="<%=RS("intCommentSeq")%>" class="d_<%=RS("intSeq")%>"></td>
							<td class="detail num"><%=intNumber%></td>
							<td class="detail lfpd"><a href="../?act=bbs&subAct=view&bid=<%=RS("strBoardID")%>&seq=<%=RS("intBoardSeq")%>#comment_id=<%=RS("intCommentSeq")%>" target="_blank"><%=GetCutDispData(GetStripTags(RS("strComment")), 40, "..")%></a></td>
