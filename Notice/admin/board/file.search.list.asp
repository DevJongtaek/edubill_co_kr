...
<%
	...
	IF strSearchField = "intSize" THEN
		IF GetNumericCheck(strSearchKeyword, "i") = False THEN strSearchKeyword = ""
	END IF

	SET RS = DBCON.EXECUTE("[ARTY30_SP_FILES_LIST_ADMIN] 'Y', '', '', '" & strSearchDate1 & "', '" & strSearchDate2 & "', '" & strSearchField & "', '" & strSearchKeyword & "' ")

	intTotalCount = RS(0)
	intPageCount = INT((intTotalCount - 1) / intPageSize) + 1
%>
<script type="text/javascript" src="../js/Calendar.js"></script>
<script type="text/javascript" src="board/js/file.search.list.js"></script>
		<div id="content">
			<form id="theForm" method="post" action="?act=filesearch">
			<input type="hidden" id="intPage" value="<%=intPage%>">
			<div id="subHead">
				<h3><%=objXmlLang("page_title_file")%></h3>
			</div>
			<p class="subHeadDesc"><%=objXmlLang("page_description_file")%></p>
...
%>
						...
						<%=GetMakeSelectForm(objXmlLang("option_search_field_file"), ",", strSearchField, "")%>
						</select>
						</li>
						<li>
			...
			<div id="subBody">
				...
				<colgroup>
					<col width="1" /><col width="22" /><col width="45" /><col width="200" /><col width="60" /><col /><col width="100" /><col width="80" /><col width="80" /><col width="50">
				</colgroup>
					<thead>
						<tr>
							<th class="lineL"></th>
							...
							<th class="bar"><%=objXmlLang("list_filename")%></th>
							<th class="bar"><%=objXmlLang("list_filesize")%></th>
							<th class="bar"><%=objXmlLang("list_down")%></th>
							<th class="bar"><%=objXmlLang("list_regdate")%></th>
							<th class="lineR bar"><%=objXmlLang("list_link")%></th>
						</tr>
					</thead>
<%
	SET RS = DBCON.EXECUTE("[ARTY30_SP_FILES_LIST_ADMIN] 'N', '" & intPage & "', '" & intPageSize & "', '" & strSearchDate1 & "', '" & strSearchDate2 & "', '" & strSearchField & "', '" & strSearchKeyword & "' ")
...
%>
						<tr>
							<td class="detail"></td>
							<td class="chk"><input type="checkbox" id="intSeq" name="intSeq" value="<%=RS("intSeq")%>"></td>
							<td class="detail num"><%=intNumber%></td>
							<td class="detail"><a href="../?act=bbs&bid=<%=RS("strBoardID")%>" target="_blank"><%=RS("strBoardTitle")%></a></td>
							<td class="detail"><% IF RS("intCommentSeq") = "0" THEN %><%=objXmlLang("text_board")%><% ELSE %><%=objXmlLang("text_comment")%><% END IF %></td>
							<td class="detail lfpd"><a href="action/?Act=down&subAct=filesearch&intSeq=<%=RS("intSeq")%>"><%=RS("strFileName")%></a></td>
							<td class="detail num"><%=GetFilesize(RS("intSize"))%></td>
							<td class="detail num"><%=RS("intDown")%></td>
							<td class="detail num"><%=REPLACE(LEFT(RS("strRegDate"),10),"-",".")%></td>
							<td class="detail"><span class="button small"><a href="../?act=bbs&subAct=view&bid=<%=RS("strBoardID")%>&page=&category=&seq=<%=RS("intBoardSeq")%>" target="_blank"><%=objXmlLang("btn_view")%></a></span></td>
						</tr>
<%
	RS.MOVENEXT
	WEND
