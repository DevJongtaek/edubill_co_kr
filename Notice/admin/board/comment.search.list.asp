...
<%
	...
	SET RS = DBCON.EXECUTE("[ARTY30_SP_COMMENTS_LIST_ADMIN] 'C', '', '', '" & strSearchDate1 & "', '" & strSearchDate2 & "', '" & strSearchField & "', '" & strSearchKeyword & "' ")

	intTotalCount = RS(0)
	intPageCount = INT((intTotalCount - 1) / intPageSize) + 1
%>
<script type="text/javascript" src="../js/Calendar.js"></script>
<script type="text/javascript" src="board/js/comment.search.list.js"></script>
		<div id="content">
			<form id="theForm" method="post" action="?act=commentsearch">
			<input type="hidden" id="intPage" value="<%=intPage%>">
			<div id="subHead">
				<h3><%=objXmlLang("page_title_comment")%></h3>
			</div>
			<p class="subHeadDesc"><%=objXmlLang("page_description_comment")%></p>
...
%>
						...
						<%=GetMakeSelectForm(objXmlLang("option_search_field_comment"), ",", strSearchField, "")%>
						</select>
						</li>
						<li>
			...
			<div id="subBody">
				...
				<colgroup>
					<col width="1" /><col width="22" /><col width="45" /><col /><col width="100" /><col width="80" /><col width="80" />
				</colgroup>
					<thead>
						<tr>
							<th class="lineL"></th>
							...
							<th class="bar"><%=objXmlLang("list_comment")%></th>
...
<%
	SET RS = DBCON.EXECUTE("[ARTY30_SP_COMMENTS_LIST_ADMIN] 'L', '" & intPage & "', '" & intPageSize & "', '" & strSearchDate1 & "', '" & strSearchDate2 & "', '" & strSearchField & "', '" & strSearchKeyword & "' ")
...
%>
						<tr>
							...
							<td class="detail lfpd"><a href="../?act=bbs&subAct=view&bid=<%=RS("strBoardID")%>&seq=<%=RS("intBoardSeq")%>#comment_id=<%=RS("intSeq")%>" target="_blank"><%=GetCutDispData(GetStripTags(RS("strContent")), 40, "..")%></a></td>
