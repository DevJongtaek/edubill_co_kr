	...
	$(document).ready(function() {
		...
		$("a[name=link_comment_file]").click(function(){

			var comment_id = $(this).attr("id");

			$("#comment_file_" + comment_id).toggle();

			if ($("#comment_file_" + comment_id + " li").length == 0){
					...
					if (responseText.length > 0){
						for(var i = 0; i < responseText.length; i++){
							if (responseText[i].downlevel == "0"){
								$("#comment_file_" + comment_id).append("<li><a>" + responseText[i].filename + " <span class=\"bubble\">[Size : " + responseText[i].filesize1 + " / Download : " + responseText[i].filedown + "</span></a></li>");
							}else{
									$("#comment_file_" + comment_id).append("<li><a href=\"?Act=bbs&subAct=filedown&bid=" + $("#extForm #bid").val() + "&seq=" + $("#extForm #seq").val() + "&comment_seq=" + comment_id + "&file_seq=" +responseText[i].seq + "\">" + responseText[i].filename + " <span class=\"bubble\">[Size : " + responseText[i].filesize1 + " / Download : " + responseText[i].filedown + "</span></a></li>");
							}
						}
		...
		$("#link_comment_show").click(function(){
			$(".commentList").toggle();
		});
