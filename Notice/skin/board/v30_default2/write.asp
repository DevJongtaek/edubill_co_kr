	...
	<div class="box">
		...
		<div class="bd bd_white">
			<div class="write_header">
				<table>
					<tr>
						<th><span><%=objXmlLang("writer")%></span></th>
						<td><input type="text" id="nickname" class="inputText nickname" title="nickname" /></td>
					</tr>
					<tr>
						<th><span><%=objXmlLang("password")%></span></th>
						<td><input type="password" id="password" class="inputText password" title="password" /></td>
					</tr>
					<tr>
						<th><span><%=objXmlLang("email")%></span></th>
						<td><input type="text" id="email" class="inputText email" title="email"></td>
					</tr>
					<tr>
						<th><span><%=objXmlLang("homepage")%></span></th>
						<td><input name="homepage" type="text" id="homepage" class="inputText homepage" title="homepage"></td>
					</tr>
					<tr>
						<th><span><%=objXmlLang("writer")%></span></th>
						<td><%=WRITE_strNickName%> (<%=WRITE_strUserID%>)</td>
					</tr>
					<tr>
						<th><span><%=objXmlLang("category")%></span></th>
						<td>
					...
					<tr>
						<th><span><%=objXmlLang("write_option")%></span></th>
						<td>
							<ul>
								<li class="opt_notice"><input type="checkbox" id="notice" value="1" /><label for="notice" class="hand"><%=objXmlLang("cmd_notice")%></label></li>
								<li class="opt_secret"><input type="checkbox" id="secret" value="1" /><label for="secret" class="hand"><%=objXmlLang("cmd_secret")%></label></li>
								<li class="opt_message"><input type="checkbox" id="message" value="1" /><label for="message" class="hand"><%=objXmlLang("cmd_message")%></label></li>
...
<table>
					<tr>
						...
						<td><input type="text" id="tag" class="inputText tag" title="tag"></td>
					</tr>
<% FOR tmpFor = 1 TO 15 %>
...
<% CASE "tel" %>
						<input type="text" class="inputText" id="<%=extraForm.field(tmpFor)%>1" size="6" maxlength="4" title="<%=extraForm.title(tmpFor)%>" />&nbsp;-&nbsp;
						<input type="text" class="inputText" id="<%=extraForm.field(tmpFor)%>2" size="6" maxlength="4" title="<%=extraForm.title(tmpFor)%>" />&nbsp;-&nbsp;
						<input type="text" class="inputText" id="<%=extraForm.field(tmpFor)%>3" size="6" maxlength="4" title="<%=extraForm.title(tmpFor)%>" />
<% CASE "textarea" %>
...
<% CASE "date" %>
						<input type="text" id="<%=extraForm.field(tmpFor)%>" class="inputTextCalendar" size="10" readonly="readonly" title="<%=extraForm.title(tmpFor)%>" /><input type="button" class="btn_calendar" onClick="calendar(event, '<%=extraForm.field(tmpFor)%>');" />
<% CASE "datetime" %>
...
<% NEXT %>
			...
			<div class="write_footer">
				<span class="button icon"><span class="add"></span><input type="submit" value="<%=objXmlLang("cmd_regist")%>" /></span>
				<span class="button icon"><span class="cancel"></span><input type="button" id="btn_cancel" value="<%=objXmlLang("cmd_cancel")%>" /></span>
			</div>
