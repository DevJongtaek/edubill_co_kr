...
<form name=form>
		...
		<td width=50% align=right>
			주문방법선택 : <select name="family" onChange="move_site(this.form)">
				<option value="index3.asp">상품코드주문
				<option value="index4.asp" selected>상품명주문
			</select>
			<select name="family2" onChange="move_site2(this.form)">
				<option value="index4.asp?orderby=pname" <%if request("orderby")="pname" then%>selected<%end if%>>상품명정렬
				<option value="index4.asp?orderby=pcode" <%if request("orderby")="pcode" then%>selected<%end if%>>상품코드정렬
			</select>
<!--
	...
	<tr>
		...
		<table cellspacing=1 cellpadding=1 width="100%" border=0 bgcolor=#BDCBE7>
			<tr height=28 bgcolor=#F7F7FF align=center>
				<td width=15%><B>코드</td>
				<td width=32%><B>상품명</td>
				<td width=20%><B>주문수량</td>
				<!-- <td width=20%><B>규격</td> -->
				<td width=13%><B>단가</td>
			</tr>
...
<!--------레이어 팝업---------->
<div id="popWindow" style="position:absolute; left:0px; top:270px; z-index:0; visibility;" onSelectStart="return false" isInfoWin="true">
...
<%if imsich_gongi<>"" then%>
<!--------레이어 팝업---------->
<div id="popWindow2" style="position:absolute; left:0px; top:400px; z-index:0; visibility;" onSelectStart="return false" isInfoWin="true">
<table width="300" height=100 cellpadding="0" cellspacing="0" border="0" bgcolor=white>
	<tr>
