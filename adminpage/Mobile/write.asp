...
<table width="310" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		...
		<td width=50% align=right>(<%if imsivatflag="y" then%>부가세포함<%elseif imsivatflag="n" then%>부가세별도<%elseif imsivatflag="a" then%>비과세<%end if%>)</td>
	</tr>
</table>

<table border="1" cellspacing="0" cellpadding="2" bordercolorlight="#999966" bordercolordark="#FFFFFF" width="310">
	<tr height="25">
...
<%End If %>
<table border="0" cellspacing="1" cellpadding="1" width=310 bgcolor=#BDCBE7>
	<tr height=28 bgcolor=#F7F7FF align=center>
		<td width=8%>코드</td>
		<td width=20%>상품명</td>
		<!-- <td width=15%>규격</td> -->
		<td width=11%>단가</td>
		<td width=8%>주문<br>수량</td>
		<td width=15%>공급가</td>
		<td width=11%>세액</td>
		<td width=10%>배달<br>수량</td>
...
<input type=hidden name=imsivatflag value="<%=imsivatflag%>">
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center>
		...
		<td>
			<input type=text size=4 maxlength=8 name=ordernum value="<%=rs("rordernum")%>" OnKeypress="onlyNumber();" ONBlur="return zeronullcheck(this);">
...
%>
	...
	<tr height=22 bgcolor=#F7F7FF align=center align=center>
		<td colspan=3>합 계</td>
		<td><%=imsihap2%></td>
		<td colspan=2  align=right><%=formatnumber(hap3+hap4,0)%></td>
		<td><%=hap2%></td>
...
%>
	...
	<tr height=22 bgcolor=#F7F7FF align=center align=center>
		<td colspan=3>합 계</td>
		<td><%=imsihap2%></td>
		<td colspan=2 align=right><%=formatnumber(rsPlushap3+rsPlushap4,0)%></td>
		<td><%=rsPlushap2%></td>
		...
		<td height=30 align=center>
			<%if imsiimsiflag="F" and flag="n" then%>
				...
				%>
					...
					<a href="cancle.asp?cancleMoney=<%=hap3+hap4%>&gotopage=<%=gotopage%>&flag=<%=flag%>&idx=<%=idx%>&searcha=<%=searcha%>&searche=<%=searche%>&searchd=<%=searchd%>&searchg=<%=searchg%>&searchh=<%=searchh%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>"><img src="../images/mobile/cancle.gif" border=0></a>
				<%end if%>
			<%end if%>
			<a href="list.asp?searcha=<%=searcha%>&searchb=<%=searchb%>&searchc=<%=searchc%>&searchd=<%=searchd%>&searche=<%=searche%>&searchf=<%=searchf%>&searchg=<%=searchg%>&searchh=<%=searchh%>"><img src="/images/admin/l_bu07.gif" border=0></a>
		</td>
	</tr>
