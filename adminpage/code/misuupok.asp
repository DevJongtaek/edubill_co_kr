<!--#include virtual="/adminpage/code/inc/fileup.asp"-->
...
<%
	...
	uploadRoot = server.MapPath("/") & "\fileupdown\misu"
	...
	do while not xRs.eof 
		...
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		if errormsg="" then
			SQL = " select idx,mi_money from tb_company where flag = '3' and bidxsub = "& bidxsub &" and idxsub = "& idxsub &" and tcode = '"& tcode &"' "
            ...
            SQL = SQL & " preye_money = "&tyemoney&""
            SQL = SQL & " where idx =" & tidx
            db.execute SQL
           

			SQL = " update tb_company set "
			SQL = SQL & " mi_money = "& tmmoney &" ,"
			SQL = SQL & " ye_money = "& tymoney &" "
			SQL = SQL & " where idx = "& tidx
			db.Execute SQL
