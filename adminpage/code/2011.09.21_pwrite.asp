...
<%
	...
	SQL = " select brandflag,dcenterflag,proflag1,proflag2,tcode,fileflag,fileflagchb,productorderweek "
	...
	if idx <> "" then
		...
		SQL = " select idx,pcode,pname,pprice,ptitle,gubun,bigo,soundfile,fileregcode,cbrandchoice,proout,qty_code,dcenterchoice,prochoice,proimage,order_weekchoice "
...
<BR>숫자 4자리
		...
		<td><input type="text" name="pprice" maxlength="7" size=10 style="ime-mode:disabled;" class="box_write" value="<%=imsipprice%>" OnKeypress="onlyNumber();">원</td>
		<td>
	...
	<tr bgcolor=#F7F7FF height=22 align=center>
	<td colspan=2><B>주문가능 요일선택 <%If idx <> "" Then %><input type="button" value="모두적용"  style="background:lightblue;" onclick="return allregsave2();"><%End If%></td>
	<td colspan=4></td>
	</tr>
	<tr bgcolor=white height=22 align=center>
		<td colspan=2>
