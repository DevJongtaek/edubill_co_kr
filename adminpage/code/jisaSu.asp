...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		<td><font color=blue size=3><B>[ 수수료 정산내역 ]</td>
	</tr>
	<tr height="40">
		<td>
			<font color=red>
☞ 오늘기준 최근 2주간 정산내역 입니다. ( 정산기간 수정 후, 최장 1개월까지 조회가능 )
<BR>
☞ 본 화면은 집계표작성 작업이 이루어지므로 시간이 오래(약 20초 정도) 걸릴 수 있습니다. 
<BR>
☞ 수수료를 정산해 주는 조건 : 배달 및 수금까지 완료된 주문 건 ( 즉, 주문내역의 배달일자에 '*' 된 건 - 다운로드 된 건)  
			</font>
		</td>
	</tr>
</table>
...
<form action="jisaSu.asp" method="POST" name=form>
	...
	<tr>
		<td nowrap width="16%" bgcolor="#F7F7FF" height="25" align=center><B>정산기간</td>
			...
			<tr>
				<td>
					...
					<input type="Text" name="searche" size="8" maxlength="8" class="box_work" value="<%=searche%>" OnKeypress="onlyNumber();" style="ime-mode:disabled;">
			        	<input type="button" name="검 색" value="검 색 " class="box_work"' onclick="javascript:stxtcheck(this.form);">
					( 년월일 ∼년월일 )
				</td>
				<td align=right>
	        			<input type="button" name="인쇄" value="인쇄" class="box_work"' onclick="javascript:posorderwin();">
	        			<input type="button" name="엑셀" value="엑셀" class="box_work"' onclick="javascript:location.href='jisaSu_excel.asp?<%=linksql%>';">
				</td>
			</tr>
		</table>
		</td>
...
<table border="0" cellspacing="0" cellpadding="0" width=100%><tr height=20><td><B>[ 체인점별 합계표 ]</td></tr></table>

<table border="0" cellspacing="1" cellpadding="1" width=100% bgcolor=#B4B4B4>
	<tr height=25 bgcolor=#EFEFEF align=center>
		<td width=5%>번호</td>
		<td width=10%>체인점코드</td>
		<td width=40%>체인점명</td>
		<td width=10%>대표자</td>
		<td width=10%>등록일자</td>
		<td width=10%>박스합계</td>
		<td width=15%>수수료합계</td>
	</tr>
...
<%
	if PrivateArrint<>"" then
		...
		for i=0 to PrivateArrint
			...
			imsihtm = imsihtm & " 		<td width='7%'>일자</td>"
			imsihtm = imsihtm & " 		<td width='10%'>상품코드</td>"
			imsihtm = imsihtm & " 		<td width='33%'>상품명</td>"
			imsihtm = imsihtm & " 		<td width='15%'>규격</td>"
			imsihtm = imsihtm & " 		<td width='10%'>단가</td>"
			imsihtm = imsihtm & " 		<td width='10%'>박스</td>"
			imsihtm = imsihtm & " 		<td width='15%'>수수료</td>"
			imsihtm = imsihtm & " 	</tr>"
			'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			...
			if i=0 or newTcode<>oldTcode then
				if i>0 then 
					...
					response.write "		<td colspan=5>소 계</td> "
