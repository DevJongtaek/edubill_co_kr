...
<%
	...
	server_name = Trim(replace(request("server_name"),"'","''"))
	<!--2013.10.25 오종택 PC명,내부IP 추가-->
		pc_name = Trim(replace(request("pc_name"),"'","''"))
		server_inname = Trim(replace(request("server_inname"),"'","''"))
    ...
    chk_gongi3 = replace(request("chk_gongi3"),"'","''")
	...
	overlapcheckuse   = request("overlapcheckuse")
	If overlapcheckuse = "" Then 
		overlapcheckuse = "n"
	End if	
	...
	SearchStartTime_1 = request("SearchStartTime_1")
	SearchStartTime_2 = request("SearchStartTime_2")
	SearchEndTime_1 = request("SearchEndTime_1")
	SearchEndTime_2 = request("SearchEndTime_2")

	if SearchStartTime_1<>"" and SearchStartTime_2<>"" and SearchEndTime_1<>"" and SearchEndTime_2<>"" then
		if len(SearchStartTime_1)=1 then
			SearchStartTime_1 = "0"&SearchStartTime_1
		elseif int(SearchStartTime_1) >= 24 then
			SearchStartTime_1 = "00"
		end if
		if len(SearchStartTime_2)=1 or int(SearchStartTime_2) >= 60 then
			SearchStartTime_2 = "00"
		end if

		if len(SearchEndTime_1)=1 then
			SearchEndTime_1 = "0"&SearchEndTime_1
		elseif int(SearchEndTime_1) >= 24 then
			SearchEndTime_1 = "00"
		end if
		if len(SearchEndTime_2)=1 or int(SearchEndTime_2) >= 60 then
			SearchEndTime_2 = "00"
		end if

		SearchStartTime = SearchStartTime_1&SearchStartTime_2
		SearchEndTime = SearchEndTime_1&SearchEndTime_2

		if len(SearchStartTime)<4 or len(SearchEndTime)<4 then
			SearchStartTime=""
			SearchEndTime=""
		else
			if (SearchStartTime >= 2460) or (SearchEndTime >= 2460) then
				SearchStartTime=""
				SearchEndTime=""
			end if
		end if
		...
		SearchEndTime=""
                    ...
                    SQL = SQL & " chk_gongi3 = '"& chk_gongi3 &"', "


					SQL = SQL & " noticeflag = '"& noticeflag &"', "
					
					SQL = SQL & " SearchStartTime = '"& SearchStartTime &"', "
					SQL = SQL & " SearchEndTime = '"& SearchEndTime &"', "

					SQL = SQL & " overlapcheckuse = '"& overlapcheckuse &"', "
					...
					SQL = SQL & " server_name = '" & server_name & "', "
					<!---->
					SQL = SQL & " pc_name = '" & pc_name & "', "
					SQL = SQL & " server_inname = '" & server_inname & "' "
