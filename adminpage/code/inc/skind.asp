...
txt_<%=barray(0,i)%> = new Array('체인점선택'<%if sarrayint<>"" then%><%for j=0 to sarrayint%>,'<%=sarray(1,j)%>'<%next%><%end if%>)
...
function Select_cate(form,v){
	var indx="0";
	var tmp = "";	
	form = document.findkindform;	//폼이름
...
//선택된 값을 뿌려줌
function set_menu(){
	document.findkindform.searchc.value='<%=searchc%>';
	Select_cate(document.findkindform,'<%=searchf%>');
	}
//-->
</script>
