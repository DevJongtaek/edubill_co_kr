...
<input type="text" name="tel3" maxlength="4" size=4 class="box_write" value="<%=imsitel3%>" OnKeypress="onlyNumber();">
	...
	<tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B><font color="blue">물류센터번호&nbsp;</td>
		<td bgcolor=white>
...
<input type="text" name="fax3" maxlength="4" size=4 class="box_write" value="<%=imsifax3%>" OnKeypress="onlyNumber();">
    ...
    <tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>입금계좌&nbsp;</td>
		<td bgcolor=white>
			은행명 : <input type="text" name="virtual_acnt3_bank" value="<%=imsivirtual_acnt3_bank%>" size=10>은행
			&nbsp;&nbsp;
			계좌번호<input type="text" name="virtual_acnt3" value="<%=imsivirtual_acnt3%>" size=20>
            &nbsp;&nbsp;
            예금주<input type="text" name="virtual_name3" value="<%=imsivirtual_name3%>" size=20>
		</td>
	</tr>
...
-->
	...
	<tr bgcolor=#F7F7FF height=22 align=left>
		...
		<td bgcolor=white>
			<select name=fileflag onchange="filechb()">
				...
				<option value="b" <%if imsifileflag="b" then%>selected<%end if%>>Text 8 (주문후 입금액 비교생성 )
				<option value="c" <%if imsifileflag="c" then%>selected<%end if%>>Text 9 (에듀빌 포맷,세트메뉴분할)
                ...
                <option value="d" <%if imsifileflag="d" then%>selected<%end if%>>Excel 4 (화일생성코드, 호차추가,주문확인건)
                <option value="e" <%if imsifileflag="e" then%>selected<%end if%>>Excel 5 (CJ 포멧, 배송일자/확인건)
			</select>
			화일생성코드 중복체크 : 
	...
	<tr bgcolor=#F7F7FF height=22 align=left>
		...
		<td bgcolor=white>
			...
			<select name="myflag_select">
                ...
                <!--<option value="3" <%if imsimyflag_select="3" then%>selected<%end if%>>당일 주문건이 있으면 추가 주문-->
                <option value="4" <%if imsimyflag_select="4" then%>selected<%end if%>>주문 후, 미수초과 주문허용
			</select>
		</td>
	</tr>
	<tr bgcolor=#F7F7FF height=22 align=left>
    ...
    <tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>직전미수금 인쇄&nbsp;</td>
		<td bgcolor=white>
			<input type="radio" name="premisuprint" value="n" <%if imsipremisuprint="n" or imsipremisuprint="" then%>checked<%end if%>>안한다 
			<input type="radio" name="premisuprint" value="y" <%if imsipremisuprint="y" then%>checked<%end if%>>한다
		</td>
	</tr>
	<tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>현재미수금 인쇄&nbsp;</td>
		<td bgcolor=white>
	...
	<tr bgcolor=#F7F7FF height=22 align=left>
		...
		<td bgcolor=white>
            ...
            <input type="radio" name="ipConFlag" value="3" <%if imsiipConFlag="3" then%>checked<%end if%>>바드미 매출처원장
		</td>
	</tr>
...
<%end if%>
	...
	<tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>물류센터 합계 보기&nbsp;</td>
		<td bgcolor=white>
			<input type="radio" name="UseDistributionTotal" value="n" <%if imsiUseDistributionTotal="n" or imsiUseDistributionTotal="" then%>checked<%end if%>>안한다	
			<input type="radio" name="UseDistributionTotal" value="y" <%if imsiUseDistributionTotal="y" then%>checked<%end if%>>한다
		</td>
	</tr>
  
    <tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>반품하기&nbsp;</td>
		<td bgcolor=white>
           
			<input type="radio" name="UseReturn" value="n" <%if imsiUseReturn="n" or imsiUseReturn="" then%>checked<%end if%> onclick="rcheck_no();">안한다	
			<input type="radio" name="UseReturn" value="y" <%if imsiUseReturn="y" then%>checked<%end if%> onclick="rcheck_ok();">한다
           
						<input type=button name=btnReturn value="반품사유" onclick="javascript:bwinopenXY('rwrite.asp?idx=<%=idx%>', 'domain', 500, 500)" <%if imsibrandflag="n" or imsibrandflag="" then%>disabled<%end if%>></td>
	</tr>
   <tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>주문내역 세액표시&nbsp;</td>
		<td bgcolor=white>
			<input type="radio" name="UseTax" value="n" <%if imsiUseTax="n" or imsiUseTax="" then%>checked<%end if%>>안한다	
			<input type="radio" name="UseTax" value="y" <%if imsiUseTax="y" then%>checked<%end if%>>한다
        </td>
	</tr>
     <tr bgcolor=#F7F7FF height=22 align=left>
		<td align=right><B>주문최소금액&nbsp;</td>
		<td bgcolor=white>
			<input type="radio" name="MinOrderCheck" value="n" <%if imsiMinOrderCheck="n" or imsiMinOrderCheck="" then%>checked<%end if%>>안한다	
			<input type="radio" name="MinOrderCheck" value="y" <%if imsiMinOrderCheck="y" then%>checked<%end if%>>한다
            &nbsp;
