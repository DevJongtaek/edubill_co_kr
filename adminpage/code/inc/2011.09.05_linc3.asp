	...
	<tr> 
		...
		<%end if%>
			...
			<select name="searchi" size="1" class="box_work">
          			...
          			<option value="3" <%if searchi = "3" then%>selected<%end if%>>정지</option>
        		</select>
...
do until rslist.EOF or rslist.PageSize<i
	...
	elseif orderflag="1" or orderflag="2" or orderflag="3" then
	...
	elseif orderflag="1" then
		imsiorderflag = "<font color='red'>미수금(정지)"
			...
			<td>
				<%if session("Ausergubun")="3" then%>
					...
					<select name=orderflag style="BACKGROUND-COLOR: #F2F1EA;">
						...
						<option value="1" <%if orderflag="1" then%>selected<%end if%> style='color:red;'>미수금
