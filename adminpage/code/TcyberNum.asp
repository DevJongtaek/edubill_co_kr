...
<%
	...
	sql = " select a.*, b.comname, b.dcode,c.dname, d.comname as jisaname, b.accountflag, d.tcode as jtcode, a.iflag, e.seq, e.hname "
	sql = sql & " from  "

	SQL = sql & " 	(select seq,Aym,hname,umoney,tcode,jtcode,wdate,idx,iflag from tAccountM where flag='1') e "
	sql = sql & " 	left outer join  "

	sql = sql & " 	( select wdate,tcode,jtcode,Aym,Aymd,iflag, "
	sql = sql & " 		sum(kmoney) as kmoney,sum(smoney) as smoney,sum(hmoney) as hmoney,idate,imoney "
	sql = sql & " 	  from tAccountM group by wdate,tcode,Aym,Aymd,idate,imoney,jtcode,iflag) a "
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		<td><font color=blue size=3><B>[ 입금관리종합 ]</td>
	</tr>
</table>

<table border="1" cellspacing="0" cellpadding="2" bordercolorlight="#999966" bordercolordark="#FFFFFF" width="100%">

<form action="TcyberNum.asp" method="POST" name=form>
	<tr>
		...
		<td nowrap width="84%" bgcolor="#FFFFFF" height="25">
			...
			<select name="searchh" size="1" class="box_work">
	          		...
	          		<option value="a.idate desc, a.tcode asc, a.Aym desc" <%if searchh="a.idate desc, a.tcode asc, a.Aym desc" then%>selected<%end if%>>입금일자
        		</select>
			...
			<input type="Text" name="searchb" size="20" maxlength="20" class="box_work" value="<%=searchb%>" onkeypress="if(event.keyCode==13){kindsearchok2(this.form);}">
	        	...
	        	<input type="button" name="초기화" value="초기화" class="box_work"' onclick="javascript:frmzerocheck(this.form,'TcyberNum.asp');">
		</td>
	</tr>
...
%>
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center onMouseOver="this.style.background='#F7F7FF'" onMouseOut="this.style.background='#FFFFFF'">
		...
		<td align=left>&nbsp;<%=rslist("hname")%><%if jisaname<>"" then%><font color=red>(<%=jisaname%>)<%end if%></td>
		<td><%=rslist("dname")%></td>
		<td><%=Aym%></td>
		...
		<%if imsidt<>"" then%>
			<%imsival2 = rslist("wdate") &"/"& rslist("tcode") &"/"& rslist("jtcode") &"/"& rslist("Aym") &"/"& rslist("Aymd") &"/"& rslist("hmoney") &"/"& rslist("seq")%>
			<td><%=imsidt%></td>
			<td align=right><%=dep_amt%>&nbsp;</td>
			<td><%if rslist("iflag")="n" then%><input type=checkbox name=cancleidx value="<%=imsival2%>"><%end if%></td>
		<%else%>
			<%imsival = rslist("wdate") &"/"& rslist("tcode") &"/"& rslist("jtcode") &"/"& rslist("Aym") &"/"& rslist("Aymd") &"/"& rslist("hmoney") &"/"& rslist("seq")%>
