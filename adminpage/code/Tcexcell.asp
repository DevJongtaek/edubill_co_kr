...
<%
	...
	sql = "       select a.*, b.comname, b.dcode,c.dname, d.comname as jisaname,b.tel1,b.tel2,b.tel3,d.tel1 as jtel1,d.tel2 as jtel2,d.tel3 as jtel3,b.accountflag, b.adminProgram "
	sql = sql & " from  "
   SQL = sql & " 	(select seq,Aym,hname,umoney,tcode,jtcode,wdate,idx,iflag from tAccountM where flag='1') e "
	...
	sql = sql & " 		sum(kmoney) as kmoney,sum(smoney) as smoney,sum(hmoney) as hmoney,idate,imoney,companynum,companyname, substring(Aymd,5,2) as wmonth,substring(Aymd,7,2) as wday "
	sql = sql & " 	  from tAccountM group by wdate,tcode,Aym,Aymd,idate,imoney,jtcode,companynum,companyname) a "
    SQL = sql & " on a.tcode = e.tcode and a.wdate=e.wdate and a.jtcode=e.jtcode and a.Aym = e.Aym "
	sql = sql & " 	left outer join  "
	sql = sql & " 	( select idx,comname,virtual_acnt2,tcode,dcode,accountflag,tel1,tel2,tel3,adminProgram from tb_company where flag='1' ) b "
...
%>
	...
	<tr height=25 align=center>
		...
		<td class="accountnum"><%=rslist("wmonth")%></td>
		<td class="accountnum"><%=rslist("wday")%></td>
		...
		<td class="accountnum" align=left><%=rslist("comname")%><%if jisaname<>"" then%><font color=red>(<%=jisaname%>)<%end if%></td>
			<td class="accountnum"><%=rslist("companynum")%></td>
			<td class="accountnum"><%=rslist("companyname")%></td>
