...
<table width='100%' border="0" align="center">
	<tr height="20">
		<td>* 정산기간 : <%=mid(searchd,1,4)%>/<%=mid(searchd,5,2)%>/<%=mid(searchd,7,2)%> ~ <%=mid(searche,1,4)%>/<%=mid(searche,5,2)%>/<%=mid(searche,7,2)%></td>
	</tr>
</table>

<table border=0><tr height=20><td><B>[ 체인점별 합계표 ]</td></tr></table>

<table border=1 width=100% bordercolordark=black bordercolorlight=black cellspacing="0" cellpadding="0">
...
<%
	if PrivateArrint<>"" then
		...
		for i=0 to PrivateArrint
			imsihtm = "<table border='0' width='100%'><tr height=20><td><B>["& PrivateArr(0,i) &" ("& PrivateArr(1,i) &")]</td></tr></table>"
			imsihtm = imsihtm & " <table border=1 width='100%' bordercolordark=black bordercolorlight=black cellspacing=0 cellpadding=0>"
