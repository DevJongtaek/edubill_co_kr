...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		<td><font color=blue size=3><B>[ 반품내역 ]</td>
	</tr>
</table>

<table border="1" cellspacing="0" cellpadding="2" bordercolorlight="#999966" bordercolordark="#FFFFFF" width="100%">

<form action="returnlist.asp" method="POST" name=form>
	<tr>
		<td nowrap width="16%" bgcolor="#F7F7FF" height="25"> &nbsp;<B>반품일자<BR> &nbsp;<B>반품정보 검색</td>
...
<table border="0" cellspacing="0" cellpadding="0" bordercolorlight="#999966" bordercolordark="#FFFFFF" width="100%">
	<tr height=30 valign=bottom>
		...
		<td width=60% align=right><font color=red>반품확인일자’에 ‘*’ 혹은 ‘반품확인’ 건은 수정이나 취소 못함.</font> &nbsp; (<%if imsivatflag="y" then%>.<%elseif imsivatflag="n" then%>.<%elseif imsivatflag="a" then%>.<%end if%>)</td>
	</tr>
</table>
...
'response.write sql &"<BR>"
	...
	if not rs.eof then
		do until rs.eof
			...
			'상품 등록 갯수(수량X)와 rs 조회된 상품 등록 갯수랑 비교하고 다르면 imsimoney=rslist("rordermoney")로 리스트에 표시 해야함.
			if taxGubun="과세" then
...
%>
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center onMouseOver="this.style.background='#F7F7FF'" onMouseOut="this.style.background='#FFFFFF'">
		<td><%=j%></td>
		<td><%=imsitcode%></td>
		<td align=left>&nbsp;<a href="Returnwrite.asp?gotopage=<%=gotopage%>&flag=<%=flag%>&idx=<%=rslist("idx")%>&searcha=<%=searcha%>&searche=<%=searche%>&searchd=<%=searchd%>&searchg=<%=searchg%>&searchh=<%=searchh%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>&old_imsihapmoney=<%=imsimoney%>"><b><%=rslist("comname2")%></a>
        ...
        <td align=right>0</td> 
		<!--<td align=right><%=formatnumber(rslist("ordermoney"),0)%>&nbsp;</td> 2011.04.06 합계금액 가져오는 방법 변경 -->
