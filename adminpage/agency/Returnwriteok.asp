...
<%
    ...
    ReturnComment = replace(request("creturnchoice")," ","")
	...
	if wflag="F" then
		if pidxarray_int<>"" then
			for i=0 to pidxarray_int
                ...
                SQL = SQL & " Returncomment = "& trim(array_ReturnComment(i)) &" ,"
...
'response.write rs_mi_money3 &"<BR>"
			''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
			sql = "update tb_company set mi_money = "& "0" &" "
...
<Script language=javascript>
    ...
    //alert("<%=smsflag%>/<%=sms_pname%>/<%=sms_pnamecnt%>/<%=hp1%><%=hp2%><%=hp3%>/<%=smsttcode1%>/<%=smsttcode2%>/<%=smsBank%>/<%=smsBankNum%>/<%=smsMoney%>/<%=smsType%> 주문정보 수정이 성공적으로 이루어졌습니다.");
    alert("반품정보 수정이 성공적으로 이루어졌습니다.");
    //location.href = "/adminpage/sms.asp?smsflag=<%=smsflag%>&sms_pname=<%=sms_pname%>&sms_pnamecnt=<%=sms_pnamecnt%>&hp1=<%=hp1%>&hp2=<%=hp2%>&hp3=<%=hp3%>&smsttcode1=<%=smsttcode1%>&smsttcode2=<%=smsttcode2%>&smsBank=<%=smsBank%>&smsBankNum=<%=smsBankNum%>&smsMoney=<%=smsMoney%>&smsType=<%=smsType%>&smsMoney2=<%=smsMoney2%>&smsapply=<%=smsapply%>";
