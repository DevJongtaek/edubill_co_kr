...
//				alert("<%=smsflag%>/<%=sms_pname%>/<%=sms_pnamecnt%>/<%=hp1%><%=hp2%><%=hp3%>/<%=smsttcode1%>/<%=smsttcode2%>/<%=smsBank%>/<%=smsBankNum%>/<%=smsMoney%>/<%=smsType%> 주문정보 취소가 성공적으로 이루어졌습니다.");
        ...
        //location.href = "/adminpage/sms2.asp?smsflag=<%=smsflag%>&sms_pname=<%=sms_pname%>&sms_pnamecnt=<%=sms_pnamecnt%>&hp1=<%=hp1%>&hp2=<%=hp2%>&hp3=<%=hp3%>&smsttcode1=<%=smsttcode1%>&smsttcode2=<%=smsttcode2%>&smsBank=<%=smsBank%>&smsBankNum=<%=smsBankNum%>&smsMoney=<%=smsMoney%>&smsType=<%=smsType%>&jsmsg=<%=jsmsg%>";
			    //
        location.href = "/adminpage/Newsms.asp?smsflag=<%=smsflag%>&sms_pname=<%=sms_pname%>&sms_pnamecnt=<%=sms_pnamecnt%>&hp1=<%=hp1%>&hp2=<%=hp2%>&hp3=<%=hp3%>&smsttcode1=<%=smsttcode1%>&smsttcode2=<%=smsttcode2%>&smsBank=<%=smsBank%>&smsBankNum=<%=smsBankNum%>&smsMoney=<%=smsMoney%>&smsType=<%=smsType%>&ordergubun=no";
