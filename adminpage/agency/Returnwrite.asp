...
<%
   ...
   ' response.Write SQL
	...
	set rs = server.CreateObject("ADODB.Recordset")
	SQL = " select pidx,idx,pcode,pname,pprice,ordernum,rordernum,flag,pcodeidx,Returncomment "
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		<td width=50%><font color=blue size=3><B>[ 반품내역 ] <!--<a href="excell2.asp?idx=<%=idx%>&searcha=<%=searcha%>&searche=<%=searche%>&searchd=<%=searchd%>&searchg=<%=searchg%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>"><img src="/images/admin/excel.gif" border=0></a>--></td>
...
<input type=hidden name=imsivatflag value="<%=imsivatflag%>">
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center>
		...
		<td><%=imsiptitle%></td>
		<td align=right>0<!--<%=formatnumber(rs("pprice"),0)%>-->&nbsp;</td>
		<td><%=rs("ordernum")%></td>
...
%>
	...
	<tr height=22 bgcolor=#F7F7FF align=center align=center>
		<td colspan=4>합 계</td>
		<td><%=imsihap2%></td>
		<td colspan=2>0<!--<%=formatnumber(hap3+hap4,0)%>--></td>
		<td><%=hap2%></td>
        <td>&nbsp;</td>
...
'		response.write sql
		...
		response.write "<tr>"
		response.write "	<td colspan=9 width=10 bgcolor=white></td>"
		response.write "	</tr>"
		do until rsdc.eof
%>
		<tr height=28 bgcolor=#F7F7FF align=center>
			...
			<td colspan=3 align=right>0<!--<%=FormatNumber(rsdc(2),0)%>--></td>
			<td></td>
		...
		<td height=30 align=center>
			<%if imsiimsiflag="F" and flag="n" then%>
				...
				%>
					...
					<a href="Returncancle.asp?cancleMoney=<%=hap3+hap4%>&gotopage=<%=gotopage%>&flag=<%=flag%>&idx=<%=idx%>&searcha=<%=searcha%>&searche=<%=searche%>&searchd=<%=searchd%>&searchg=<%=searchg%>&searchh=<%=searchh%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>"><img src="Returncancle.gif" border=0></a>
				<%end if%>
			<%end if%>
