...
'	SQL = SQL & " and tcode = '"& ccode &"' "
	...
	SQL = SQL & " b.order_weekgubun,b.order_weekchoice,b.order_checkStime,b.order_checkEtime,a.myflag, b.mi_money, b.ye_money,a.tel1,a.tel2,a.tel3,a.sclose,a.proflag1,a.myflag_select,a.premisuprint, b.preye_money,a.fax1,a.fax2,a.fax3 "
   ...
   ' response.Write sql
    ...
    rs_premisuprint = rs(22)
    rs_preye_money = rs(23)
    rs_fax1     = rs(24)
		rs_fax2     = rs(25)
		rs_fax3     = rs(26)
		rs_fax = rs_fax1&"-"&rs_fax2&"-"&rs_fax3
	else
		imsinum = 0
	end if
	rs.close
	...
	if orderflag="1" or orderflag="2" or orderflag="3" Or orderflag = "6" then
		...
		If orderflag="1" And bcode = "19209" Then 
			'response.write "	alert(""현재 미수금 정지상태 입니다.\n\n지엔푸드 영업관리팀이나 담당자에게 문의 바랍니다.!\n지엔푸드 : " + gnfood + "(내선:5)" + """);"	
			response.write "	alert(""현재 주문차단 정지상태 입니다.\n\n지엔푸드 영업관리팀에 문의 바랍니다.!\n지엔푸드 : 1661-9481"");"	
		ElseIf orderflag="6" And bcode = "19209" Then 
			response.write "	alert(""현재 시즈닝차단 정지상태 입니다.\n\n플러스원이나 담당자에게 문의 바랍니다.!\n플러스원 : " + pluson + """);"	
...
'response.end
	...
	select case imsinum
		...
		case else
            ...
            session("AAfax")      = rs_fax			'물류센터연락처
            session("AAcomname2") =  username1
			 session("AAcomname3") =  username2
			'가맹점코드 저장 
			If imsi_save = "on" Then 
				imsi_bccode = bccode
			Else 
				imsi_bccode = ""
			End If 
			response.write "<Script language=javascript>"
			response.write "	function setCookie( name, value, expiredays ){  "
			response.write "	    var todayDate = new Date(); "
			response.write "	    todayDate.setDate( todayDate.getDate() + expiredays ); "
			response.write "	    document.cookie = name + ""="" + escape( value ) + ""; path=/; expires="" + todayDate.toGMTString() + "";"" ; "
			response.write "	} "
			response.write "	setCookie(""user_id"", """ & imsi_bccode & """, 100); "
			response.write "</Script>"
			...
			if rs_myflag="y" then	'여신:미수 구분
				If rs_myflag_select = "3" And orderYN > 0 Then '3이면 오늘 주문건이 있으면 추가 주문 가능
                    ...
                    if rs_premisuprint = "y" then

				
					imsijavamsg = imsijavamsg & "\n\n        직전미수금액 : "& FormatNumber(rs_preye_money,0) &" 원"
					    ...
					    imsijavamsg = imsijavamsg & "\n\n        현재미수금액 : "& FormatNumber(rs_mi_money,0) &" 원"
					    If bcode <> "25782" Then 
					...
					if session("AAproflag")="1" then	'주문방법 1:텍스트 2:서브매뉴
						...
						If mobile = "mobile" Then 
							response.write "	location.href = ""/adminpage/mobile/index4.asp?gongi=1"";"
						Else 
						...
						'''''''모바일
						If mobile = "mobile" Then 
							response.write "	location.href = ""/adminpage/mobile/mAgencyOrderFRM.asp?gongi=1&basket=" & rsResetBasket &""";"
						Else 
     ...
     if rs_premisuprint = "y" then
					...
					if rs_ye_money < rs_mi_money then	'여신 < 미수   : 주문은 받지 않고 Comment 입력만 받음
						...
						response.write "	alert("""&session("AAcomname")&"에서 로그인 하셨습니다."& imsijavamsg &""");"
						If mobile = "mobile" Then 
							response.write "	location.href = ""/adminpage/mobile/comment.asp?gongi=1"";"
						Else 
			...
			if session("Aordertimeout") = "y" then
				...
				If mobile = "mobile" Then 
					response.write "	location.href = ""/adminpage/mobile/list.asp"";"
				Else 
				...
				if session("AAproflag")="1" then	'주문방법 1:텍스트 2:서브매뉴
					...
					''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
					If mobile = "mobile" Then 
						response.write "	location.href = ""/adminpage/mobile/mAgencyOrderFRM.asp?gongi=1"";"
					Else 
    'location.href='AgencyOrderCartFRM.asp?btnsee=y'
