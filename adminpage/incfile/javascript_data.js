...
function dcentercheck_no(){
    form.btndcenter.disabled=true;
}


function rcheck_ok(){
    form.btnReturn.disabled=false;
}

function rcheck_no(){
    form.btnReturn.disabled=true;
}
...
function AutoAddr(key) {
    ...
    var pop = window.open("/DB/juso.asp", "pop", "width=570,height=420, scrollbars=yes, resizable=yes");
}
function jusoCallBack(roadFullAddr, roadAddrPart1, addrDetail, roadAddrPart2, engAddr, jibunAddr, zipNo, admCd, rnMgtSn, bdMgtSn) {
  ...
  //  document.getElementById('m_addr1').value = "";
    ...
    //document.getElementById('txt_AddressDetail').value = "";

    document.getElementById('m_addr1').value = "";
    document.getElementById('m_addr2').value = "";
    document.getElementById('m_post1').value  = "";
    document.getElementById('m_post2').value  = "";

    var address = roadFullAddr.split(" ")
    document.getElementById('m_addr1').value = address[0] + " " + address[1];
  //  document.getElementById('txt_City').value = address[1];
    for (var i = 2; i < address.length; i++) {
        document.getElementById('m_addr2').value += address[i] + " ";
    }
    document.getElementById('m_post2').value  =zipNo.substr(2,3);
    document.getElementById('m_post1').value = zipNo.substr(0,2);
    //document.getElementById('m_post1').value = zipNo;
    //document.getElementById('m_post1').value = zipNo;
}
//function AutoAddr(key){
...
function comdel1(frm) {
	var msg;
	msg=confirm("정말 삭제하겠습니까?\n\n삭제 하시면 지사/체인점/주문내역/상품내역/전표내역등 모든 내역이 함께 삭제됩니다.\n\n그래도 삭제 하시겠습니까?");
	if(msg) {
		form.dflag.value="1"
		form.submit() ;
		return false ;
...
//  alert(form.document.getElementById('rordernum').value);
	//if (form.rordernum.value=="") {
	//	alert("배달수량을 입력하여 주시기바랍니다.") ;
	//	form.rordernum.focus() ;
	//	return false ;
