...
%>
  ...
  <table width= 600 border="0" cellpadding="3" cellspacing="2">
    <tr> 
      <td class="tblOuter" WIDTH=150 bgcolor="#aa8c9b" align="center"><font face="굴림" size="2" color="#FFFFFF">전송될시간</font></td>
      <td class="tblOuter" WIDTH=100 bgcolor="#aa8c9b" align="center"><font face="굴림" size="2" color="#FFFFFF">수신자</font></td>
      <td class="tblOuter" bgcolor="#aa8c9b" align="center"><font face="굴림" size="2" color="#FFFFFF">내용</font></td>
      <td class="tblOuter" WIDTH=30 bgcolor="#aa8c9b" align="center"><font face="굴림" size="2" color="#FFFFFF">선택</font></td>
	</tr>
	
    <tr> 
      <td colspan=4 align=center class="tblOne"><font face="굴림" size="2">예약된 메시지가 없습니다.</font></td>
	</tr>
	</table>
	</td></tr>
	</table>
...
<FORM name="mainform" method="POST" action="DeleteReservMail.asp">
  ...
  <table width= 600 border="0" cellpadding="3" cellspacing="2">
    ...
    <tr> 
      ...
      <td align="center"><font face="굴림" size="2"><a href="ViewReservMail.asp?msgkey=<%=DbRec("tran_etc4")%>&page=<%=pg%>&startpage=<%=startpage%>"><%=DbRec("tran_msg")%></a></font></td>
  ...
  <tr>
	...
	[<a href="/삭제" onClick="calc_del(document.mainform);return false;"><font face="굴림" size="2">삭제</font></a>]
	[<a href="/모두선택" name="toggle" onClick="toggle_all(document.mainform);return false;"><font face="굴림" size="2">모두선택</font></a>] 
<% if totpage>ten then %> 
	<% if startpage=1 then %>
		</font><font color="#808000" size="2" face="굴림">Prev</font><font color="#000000" size="2">  
	<%else%> 
		<a href="ReservMailbox.asp?page=1&amp;startpage=1" >처음</a>
		...
		<%else%>
			...
			<%else%>
				...
				<%if (lastpg mod ten)=0 then %>
					<a href="ReservMailbox.asp?page=<%=lastpg%>&amp;startpage=<%=(lastpg\ten)*ten-(ten-1)%>" >마지막</a>
				<%else%>
					<a href="ReservMailbox.asp?page=<%=lastpg%>&amp;startpage=<%=(lastpg\ten)*ten+1%>" >마지막</a>
				<%end if%>
			<%end if%>
		<%end if%>
	<%else%></font>
