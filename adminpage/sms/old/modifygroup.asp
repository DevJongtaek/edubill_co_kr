...
<%
strgpk = Request("gpk")
sql = "select * from em_addr_group where PK = '"&strgpk&"'"
...
<tr>
<td align="center"><font face="굴림" size="3"><b>그룹 고치기</b></font></td>
</tr>
</table><br><br>
<!-------------그룹 이름 입력 시작----------->
<form method="post" name="form" action="modifygroup_exe.asp" onSubmit="return valid(this)">
<table border=0 cellpadding=0 cellspacing=0 width="300">
<tr>
...
<td>&nbsp;<input type=text name="gname" size="10" value="<%=rs("groupName")%>" maxlength="16"><font size="2"><strong><font color="#FF0000">*</font></strong></font></td>
</tr><input type="hidden" name="gpk" value="<%=rs("PK")%>">
</table><br><br>
<!--------------그룹 이름 입력 끝----------->
<!--------------구성원 선택 시작------------>
...
%>
<% while not rs.eof %>
<option value="<%=rs("pk")%>"><%=rs("personName")%>
</option>
<% 
rs.movenext 
wend
%>
	...
	<td class="tblZero" align=center>
		<select multiple name="sel_person[]" size="6" onDblClick="to_left(this.form)">	
		<%
		sql1 = "SELECT gr.personPK, gp.personName, gr.personPK FROM em_addr_group_person gr, em_addr_person gp WHERE gp.userId='" & officenum & "' and gr.groupPK ='" & strgpk & "' and gr.personPK=gp.PK"
		set rs = DBConn.Execute(sql1)
		%>
		
		<% while not rs.eof %>
		<option value="<%=rs("personPK")%>"><%=rs("personName")%></option>
		<% 
		rs.movenext 
		wend
		%>
