...
<%
strpk = Request("pk")
sql = "select * from em_addr_person where PK = '"&strpk&"'"
set rs = DBConn.Execute(sql)
%>
<body bgcolor="#FFFFFF">
<table border=0 cellpadding=0 cellspacing=0 width="300">
<tr>
<td align="center"><font face="굴림" size="3"><b>개인 정보 수정</b></font></td>
</tr>
<table border=0 cellpadding=0 cellspacing=0 width="300">
<tr>
<td><font face="굴림" size="2">주소록에 등록되어 있는 개인의 정보를 수정할 수 있습니다. 
아래에 수정하고 싶은 사람의 정보를 입력하신후, 
특정그룹에 포함하시고 싶으면 해당 그룹을 선택하여 주십시오.</font>
</td>
</table><br>
<form action="modifyusr_exe.asp" method="post" onSubmit="return valid(this)">
  <table cellpadding=2 cellspacing=1 border="0" align="center" width="300">
    ...
    <tr>
      ...
      <td>
    <input type="text" name="name" size="12" value="<%=rs("personName")%>" maxlength="16">
      <font size="2"><strong><font color="#FF0000">*</font></strong></font></td>
    </tr><input type="hidden" name="pk" value="<%=rs("PK")%>">
    	
    <tr>
      <td bgcolor="#aa8c9b" align="center"><font face="굴림" size="2" color="#FFFFFF">핸드폰</font></td>
      <td>
      	<input type=text name="pphone" size=20 maxlength="13" value="<%=rs("phone")%>">
    	</td>
    </tr>    
  </table>
  ...
  <table cellpadding=2 cellspacing=1 border="0" align="center" width="300">		
	...
	<tr>
	  ...
	  <select name="total_group" size=6 multiple onDblClick="to_right(this.form)">
		...
		%>
		<% while not rs.eof %>
		<option value="<%=rs("PK")%>"><%=rs("groupName")%>
		</option>
		<% 
		rs.movenext 
		wend
	...
	<td class="tblZero" align=center>
		<select multiple name="sel_group[]" size="6" onDblClick="to_left(this.form)">
		<%
		sql1 = "SELECT gr.groupPK, gg.groupName, gr.personPK FROM em_addr_group_person gr, em_addr_group gg WHERE gg.userId='" & officenum & "' and gr.personPK ='" & strpk & "' and gr.groupPK=gg.PK"
		set rs = DBConn.Execute(sql1)
		%>
		<% =sql1 %>
		<% while not rs.eof %>
		<option value="<% =rs("grouppk")%>"><% =rs("groupname")%></option>
		<% 
		rs.movenext 
		wend
		%>
