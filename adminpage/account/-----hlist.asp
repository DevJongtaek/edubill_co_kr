...
<%
	...
	if searcha = "" then
		searcha = replace(left(now()-365,7),"-","")
	end if
	if searchb = "" then
	...
	SQL = " select a.seq,a.Aym,a.hname,a.umoney,a.tcode,a.textBigo,b.Anum,b.kmoney,b.smoney,b.hmoney, c.accountflag from "
	SQL = sql & " (select seq,idx,Aym,hname,umoney,tcode,textBigo,wdate from tAccountM where flag='1' and "& sendSQL &" ) a "

	SQL = sql & " left outer join "
	SQL = sql & " (select wdate,sum(Anum) as Anum,sum(kmoney) as kmoney,sum(smoney) as smoney,sum(hmoney) as hmoney from tAccountM where "& sendSQL &" group by wdate) b "
	SQL = sql & " on a.wdate = b.wdate "
...
function taxpage(seqval){
	window.open ('taxpage2.asp?seq='+seqval,'taxID','toolbar=no,menubar=no,scrollbars=yes,resizable=yes,width=700,height=700');
	return false ;
}
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	<tr height="47">
		<td><font color=blue size=3><B>[ 세금계산서 발행 ]</td>
	</tr>
</table>
...
%>
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center onMouseOver="this.style.background='#F7F7FF'" onMouseOut="this.style.background='#FFFFFF'">
		...
		<%end if%>
		<td><%=rslist("textBigo")%></td>
		<td>
			<%if accountflag="y" then%><input type="button" name="세금" value="세금" class="box_work" onclick="javascript:taxpage('<%=rslist("seq")%>');"><%end if%>
