...
<%
	...
	SQL = sql & " (select seq,Aym,hname,umoney,tcode,wdate from tAccountM where flag='1' and "& sendSQL &" ) a "
	...
	SQL = sql & " (select wdate,tcode,sum(Anum) as Anum,sum(kmoney) as kmoney,sum(smoney) as smoney,sum(hmoney) as hmoney from tAccountM group by wdate,tcode) b "
	SQL = sql & " on a.tcode = b.tcode and a.wdate=b.wdate "
	SQL = sql & imsiorderby
	'rslist.PageSize=20
'response.write sql
...
<form action="backuplist2.asp" method="POST" name=form>
