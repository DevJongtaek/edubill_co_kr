...
<%
	...
	'xSql = "SELECT [회원사코드],[지사코드],[flag],[회원사명],[이용료],[체인점수],[비고] FROM [" & xSheetName & "$]" 
	...
	do while not xRs.eof 
		...
		if len(xRs(1))=4 then
			if idx<>"" then
				SQL = " select idx,comname,name,addr,addr2,uptae,upjong,companynum1,companynum2,companynum3 from tb_company where flag = '2' and bidxsub = "& idx &" and tcode = '"& xRs(1) &"' "
				Set Rs = Db.Execute(SQL)
				if not rs.eof then
					company       = rs(1)
					companyName   = rs(2)
					...
					companyUptae  = ""
					companyUpjong = ""
					errormsg = errormsg & "지사코드틀림/"
				end if
				rs.close
			else
				errormsg = errormsg & "지사코드없음/"
			end if
		end if
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		errormsg = errormsg & FunNullMsg(trim(xRs(2)),"flag없음/")
		errormsg = errormsg & FunNullMsg(trim(xRs(3)),"회원사명없음/")
		errormsg = errormsg & FunNullMsg(trim(xRs(4)),"이용료없음/")
		errormsg = errormsg & FunNullMsg(trim(xRs(5)),"체인점수없음/")
		'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		flag   = xRs(2)
		hname  = xRs(3)
		umoney = xRs(4)
		...
		if errormsg="" then
			...
			imsisql1 = "idx,tcode,jtcode,Aym,Aymd,flag,hname,umoney,Anum,kmoney,smoney,hmoney,textName,textPname,textPname2,textBigo,company,companynum,companyName,companyAddr,companyUptae,companyUpjong,wdate,filename"
			imsisql2 = " "& idx &",'"& tcode &"','"& jtcode &"','"& Aym &"','"& Aymd &"','"& flag &"','"& hname &"',"& umoney &","& Anum &","& kmoney &","& smoney &","& hmoney &",'"& textName &"','"& textPname &"',"
