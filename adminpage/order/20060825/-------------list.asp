...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	SQL = " select idx,usercode,comname1,comname2,orderday,ordermoney,carname,ordering,flag,wdate,rordermoney,deflagday,sendhpnum,FtpFile,isnull(order_cmt,'') as order_cmt"
	SQL = sql & " from tb_order "
	...
	if searchh = "1" then		'유
		SQL = sql & " and ((FtpFile <> '' and FtpFile is not null) or (order_cmt <> '' and order_cmt is not null))"
	elseif searchh = "2" then	'무
		SQL = sql & " and ((FtpFile = '' or FtpFile is null) and (order_cmt = '' or order_cmt is null))"
	end if
...
%>
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center onMouseOver="this.style.background='#F7F7FF'" onMouseOut="this.style.background='#FFFFFF'">
		<td><%=j%></td>
		<td><%=imsitcode%></td>
		<td align=left>&nbsp;<a href="write.asp?gotopage=<%=gotopage%>&flag=<%=flag%>&idx=<%=rslist("idx")%>&searcha=<%=searcha%>&searchb=<%=searchb%>&searchc=<%=searchc%>&searchd=<%=searchd%>&searche=<%=searche%>&searchf=<%=searchf%>&searchg=<%=searchg%>&searchh=<%=searchh%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>"><b><%=rslist("comname2")%></a><%if imsiiconfile<>"" then%><a href="/fileupdown/<%=imsiiconfile%>" target=_blank><img src="icon.gif" border=0></a><%end if%>
<%if imsiorder_cmt<>"" then%><a href="#" onclick="window.open('order_cmt.asp?idx=<%=rslist("idx")%>' , 'blank<%=i%>','left=100, top=100, width=400, height=300, scrollbars=yes, toolbar=no');"><img src="/images/coment.gif" border=0></a><%end if%></td>
		<td><%=imsidaytime%></td>
