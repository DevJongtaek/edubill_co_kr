...
<%
	...
	if session("Ausergubun")="3" then
		codeFlagSql = " and substring(tb_order.usercode,1,10) = '"& left(session("Ausercode"),10) &"' and  ISNULL(Rgubun,0) != 1  "
	else
		codeFlagSql = " and substring(tb_order.usercode,1,5) = '"& left(session("Ausercode"),5) &"' and  ISNULL(Rgubun,0) != 1  "
	end if
	'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	if session("Ausergubun")="3" then
		codeFlagSql2 = " where substring(usercode,1,10) = '"& left(session("Ausercode"),10) &"' and  ISNULL(Rgubun,0) != 1  "
	else
		codeFlagSql2 = " where substring(usercode,1,5) = '"& left(session("Ausercode"),5) &"' and  ISNULL(Rgubun,0) != 1  "
	end if
	'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	'지호 삼계탕 배달확정 건수 확인 
		...
		SQL = SQL + " where substring(usercode,1,5) = '10339' "
		SQL = SQL + " and convert(datetime, left(idx, 4) + '-' + substring(idx, 5, 2) + '-' + substring(idx, 7, 2) + ' ' + substring(idx, 9, 2) + ':' + substring(idx, 11, 2) + ':' + substring(idx, 13, 2)) >= convert(datetime, left('20120711000000', 4) + '-' + substring('20120711000000', 5, 2) + '-' + substring('20120711000000', 7, 2) + ' ' + substring('20120711000000', 9, 2) + ':' + substring('20120711000000', 11, 2) + ':' + substring('20120711000000', 13, 2)) "
		SQL = SQL + " and convert(datetime, left(idx, 4) + '-' + substring(idx, 5, 2) + '-' + substring(idx, 7, 2) + ' ' + substring(idx, 9, 2) + ':' + substring(idx, 11, 2) + ':' + substring(idx, 13, 2)) <= convert(datetime, left('20120712235959', 4) + '-' + substring('20120712235959', 5, 2) + '-' + substring('20120712235959', 7, 2) + ' ' + substring('20120712235959', 9, 2) + ':' + substring('20120712235959', 11, 2) + ':' + substring('20120712235959', 13, 2)) "
	...
	'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
		...
		SQL = sql & " where   ISNULL(a.Rgubun,0) != 1 and  substring(a.usercode,1,5)  = b.bidxsub "
...
'response.end
	...
	'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	If left(session("Ausercode"),5) = "25782" Then 
		message = Year(now) & "년" & Month(now) & "월" & Day(now) & "일 \n배달 확정 건수 : " & regCount & " 건"
		redirect = "list.asp?gotopage="&gotopage&"&searche="&searche&"&searchd="&searchd&"&searchg="&searchg&"&fclass1="&fclass1&"&sclass2="&sclass2&"&tclass3="&tclass3&"&searchc="&searchc&"&searchf="&searchf&"&searchh="&searchh&"&searchk="&searchk&"&searchl="&searchl
		response.write "<Script language=javascript>"
		response.write "	alert(""" & message & """);"
		response.write "	location.href=""" & redirect & """;"
