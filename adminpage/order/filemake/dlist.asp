...
<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#ffffff" align="center">
	...
	<tr height="6">
		<td><font color=red size=2>※.'주문내역'의 주문 건을 지정한 화일형식(TEXT,EXCEL)으로 일괄 생성시켜주는 기능.(ERP와 연동 - 더존,CJ,바드미 등)   </td>
	</tr>
</table>
...
<form action="dnfilemake.asp" method="POST" name=findkindform2>
...
<table border="0" cellspacing="0" cellpadding="0" bordercolorlight="#999966" bordercolordark="#FFFFFF" width="100%">
	<tr height=30 valign=bottom>
		<td width=70%>* 전체 <B><%=rslist.recordcount%></b>개의 데이타가 있습니다.</td>
		<td width=30% align=right><%if imsiregok="y" then%><a href="dnfilemake.asp?imsifileflag=<%=imsifileflag%>" onClick="CallJS('Demo()')"><img src="/images/l_bu22.gif" border=0></a><%end if%></td>
	</tr>
</table>
<%end if%>
...
%>
	...
	<tr height=22 bgcolor=#FFFFFF align=center align=center onMouseOver="this.style.background='#F7F7FF'" onMouseOut="this.style.background='#FFFFFF'">
		...
		<td><a href="down.asp?fname=<%=rslist("filename")%>"><%=rslist("filename")%></a></td>
		<td><%if imsidjflag="yyy" then%><a href="#" onclick="javascript:location.href='excell.asp?searcha=<%=rslist("start_date")%>&searchb=<%=rslist("end_date")%>';"><img src="/images/admin/excel.gif" border=0></a><%end if%></td>
	</tr>
