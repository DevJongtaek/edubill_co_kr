<%
	...
	Response.AddHeader "Content-Disposition","attachment;filename=지사 상품별 집계표.xls"
%>
...
<%
    ...
    if session("Ausergubun")="3" then
		SQL = sql & " and idx = '"& right(session("Ausercode"),5) &"' "
	end If 
    'response.write  sql
	rs1.Open SQL, db, 1
	...
	SQL = SQL & " and substring(usercode,1,5) = '"& left(session("Ausercode"),5) &"' "
     if session("Ausergubun")="3" then
		SQL = sql & " and substring(a.usercode,6,5) = '"& right(session("Ausercode"),5) &"' "
	end If     
