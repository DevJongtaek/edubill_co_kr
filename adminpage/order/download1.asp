...
'Server.ScriptTimeOut = 1000000
	...
	if imsifileflag="7" then
		minmaxsql = "select max(C.pidx), min(C.pidx), max(C.idx), min(C.idx), count(DISTINCT b.idx), count(DISTINCT B.idx)"
		...
		'20060825수정
		minmaxsql = minmaxsql & " AND C.rordernum > 0 "
...
if imsifileflag="3" then
	...
	fs.CreateTextFile "D:\Webhosting\2004VIVA\edubill_co_kr_new\fileupdown\txt\" & yyyymmdd_file & ".txt",true 
	Set objFile = fs.OpenTextFile("D:\Webhosting\2004VIVA\edubill_co_kr_new\fileupdown\txt\" & yyyymmdd_file & ".txt",8) 
...
elseif imsifileflag="7" then
	...
	Do Until rs.EOF
		...
		'USql = Usql & " where idx = '" & Trim(rs("idx")) & "'"
		db.execute(USql)
...
'response.end
	...
	set db=nothing
    	...
    	objStream.Open
    	objStream.Type = 1
    	objStream.LoadFromFile "D:/Webhosting/2004VIVA/edubill_co_kr/fileupdown/txt/" & file        '//경로 입니다.
