...
<form action="Returnlist.asp" method="POST" name=form>
	<tr>
		<td nowrap width="16%" bgcolor="#F7F7FF" height="25"> &nbsp;<B>반품일자<BR> &nbsp;<B>주문정보 검색</td>
...
<%if session("Ausergubun")="2" or session("Ausergubun")="3" then%>
	<%if session("Auserwrite")="y" then%>
		...
		<a href="Returnallreg.asp?searchc=<%=searchc%>&searchf=<%=searchf%>&searchh=<%=searchh%>&searcha=<%=searcha%>&searche=<%=searche%>&searchd=<%=searchd%>&searchg=<%=searchg%>&fclass1=<%=fclass1%>&sclass2=<%=sclass2%>&tclass3=<%=tclass3%>&searchk=<%=searchk%>&searchl=<%=searchl%>&searchm=<%=searchordertime%>" onClick="CallJS('Demo()')"><img src="/images/admin/l_bu19_1.gif" border=0></a>
