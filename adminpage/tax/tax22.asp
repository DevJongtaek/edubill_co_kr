...
<%
	...
	if session("Ausergubun")="3" and session("Adcenteridx")<>"" and len(session("Adcenteridx")) > 1 then
		SQL = sql & " from (select b.* from tb_order_product a, tb_order b where a.idx = b.idx    AND ISNULL(b.Rgubun,0) != 1  and a.dcenterchoice = '"& trim(session("Adcenteridx")) &"') c "
	else
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	if vatflag="y" then						'포함
		ordermoney     = imsirordermoney			'수정전공급가액
		ordermoney_vat = 0					'부가세
		ordermoney_hap = ordermoney+ordermoney_vat		'부가세포함

	else								'별도
		ordermoney     = imsirordermoney			'수정전공급가액
		ordermoney_vat = 0					'부가세
		ordermoney_hap = CDbl(ordermoney)+ordermoney_vat	'부가세포함
	end if
	...
	<tr> 
		...
		<table width="620" border=0" cellspacing="0" cellpadding="0">
			...
			<tr> 
				...
				<td class="tdbd_blue_top1left1" height="32" width="87" align="center"><%=b_name%>
				<%if imsisealname<>"" then%>
<img src="/fileupdown/logo/<%=imsisealname%>" width=32 height=32 border="0">
        <%else%>
				<font color="#4956C7">(인)</font>
				<%end if%>
	...
	<tr> 
		...
		<table width="620" border=0 cellspacing="0" cellpadding="0">
			...
			<tr> 
				...
				<td class="tdbd_blue_top1left11" height="32" width="87" align="center"><%=b_name%>
				
				<%if imsisealname<>"" then%>
<img src="/fileupdown/logo/<%=imsisealname%>" width=32 height=32 border="0">
        <%else%>
				<font color="#F81840">(인)</font>
				<%end if%>
