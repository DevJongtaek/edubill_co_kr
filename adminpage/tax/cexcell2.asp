...
<%
	...
	sql = " select *,c.virtual_acnt3,c.virtual_acnt3_bank from tb_virtual_acnt A "
	...
	SQL  = sql & " left join tb_company c "
	SQL = sql & " on a.cporg_cd = c.cporg_cd "
	
	SQL = sql & " where a.cporg_cd = '"& btcode &"' "

	if session("Ausergubun")="4" and ctcode<>"" then	'체인점
		SQL = sql & " and a.chancode = '"& ctcode &"' "
	end if

	if searchd<>"" then
		SQL = sql & " and a.tr_il >= '"& searchd &"' "
	end if
	if searche<>"" then
		SQL = sql & " and a.tr_il <= '"& searche &"' "
	end if
	...
	if searchc="1" then
		SQL = sql & " and (a.ye_money-a.mi_money) >= 0 "
	elseif searchc="2" then
		SQL = sql & " and (a.ye_money-a.mi_money) < 0 "
	end if

	SQL = sql & " order by a.tr_il desc, a.tr_si desc, a.channame asc "
...
%>
	...
	<tr height=25 align=center>
		...
		<td><%=imsitmsg%>&nbsp;</td>-->
		<td><%=rslist("virtual_acnt3_bank")%>은행</td>
		<td  class="accountnum"><%=rslist("virtual_acnt3")%></td>
		<td class="accountnum">00<%=rslist("chancode")%></td>
		<td><%=rslist("dep_amt")%></td>
