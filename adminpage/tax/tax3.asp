...
'choiceDcenter=10049
	...
	'공급자-본사/지사
	set rs = server.CreateObject("ADODB.Recordset")
	SQL = " select vatflag,misuprint,premisuprint,cyberNum "
	SQL = sql & " from tb_company"
	SQL = sql & " where idx = "& busercode &" "
	rs.Open sql, db, 1
	vatflag   = rs(0)
	...
	'공급받는자-체인점
	set rs = server.CreateObject("ADODB.Recordset")
	SQL = " select comname,mi_money,preye_money,virtual_acnt_bank,virtual_acnt"
	SQL = sql & " from tb_company"
   ...
   ' response.Write sql
	...
	if not rs.eof then
        ...
        virtual_acnt_bank_print = rs(3)
        virtual_acnt_print = rs(4)
	else
		c_comname = ""
		misuprint_money = 0
    premisuprint_money = 0
	...
	SQL = sql & " and a.idx in ( "
       ...
       sql = sql & " AND ISNULL(Rgubun,0) != 1 "
		if session("Ausergubun")="3" then
			...
			SQL = sql & " and substring(usercode,1,5) = '"& left(session("Ausercode"),5) &"' "
            'SQL = sql & " and usercode = '"& imsiusercode &"' "
		end if
		...
		<table width="98%" border=0 cellspacing="0" cellpadding="0" align=center>
			...
			<tr>
				...
				<table width="100%" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td colspan="4">
                            입금계좌 : <%=virtual_acnt_bank_print %>은행 &nbsp;<%=virtual_acnt_print %>&nbsp; (<%=b_comname %>)
