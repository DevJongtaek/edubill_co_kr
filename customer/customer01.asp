...
<%end if%>
<a href="customer01_01.asp?idx=<%=rs("idx")%>&uid=<%=uid%>&gotopage=<%=gotopage%>"><%=txtbold%><%=rs("title")%></a>
...
%>
	...
	<tr>
		<td height="50" align="center">
			...
			Else %>
				<a href="customer01.asp?GotoPage=1&uid=<%=uid%>&searcha=<%=searcha%>&searchb=<%=searchb%>">첫페이지</a>&nbsp;
				<a href="customer01.asp?GotoPage=<%=blockPage-10%>&uid=<%=uid%>&searcha=<%=searcha%>&searchb=<%=searchb%>">이전 10개</a>&nbsp;[&nbsp;		
			<% End If
			i=1
			Do Until i > 10 or blockPage > rs.pagecount
				...
				<%Else%> 
					<a href="customer01.asp?GotoPage=<%=blockPage%>&uid=<%=uid%>&searcha=<%=searcha%>&searchb=<%=searchb%>">[<font color=blue><%=blockPage%></font>]</a>			
				<%End If
			...
			Else %> 
				...
				&nbsp;<a href="customer01.asp?GotoPage=<%=rs.pagecount%>&uid=<%=uid%>&searcha=<%=searcha%>&searchb=<%=searchb%>">마지막</a>		
			<% End If %>
